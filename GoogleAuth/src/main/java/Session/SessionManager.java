	...
	public static void createSession(String username, String sessionId)
	{
		...
		try {
			...
			if (!con.isClosed()) {
				...
				int userId = DatabaseConnection.getUserID(username, con);
				System.out.println("userId:" + userId);

				PreparedStatement pstmt;
				Timestamp currentTimestamp = new java.sql.Timestamp(Calendar.getInstance().getTime().getTime());

				pstmt = con.prepareStatement("insert into demoprj.session_tracker (session_id, user_id, create_date) values (?, ?, ?)");
